...
export function DemandaColumn({ id, title, color, demandas, onDemandaClick }: DemandaColumnProps) {
  const { setNodeRef, isOver } = useDroppable({ id });

  return (
    <div
      ref={setNodeRef}
      className={`flex flex-col w-72 rounded-2xl border transition-all shrink-0 overflow-hidden min-h-[500px] ${
        isOver
