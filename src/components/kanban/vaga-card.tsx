...
export function VagaCard({ vaga, onClick, isOverlay }: VagaCardProps) {
  ...
  const diffInDays = Math.floor(
    (new Date().getTime() - new Date(vaga.data_etapa_atual).getTime()) /
      (1000 * 3600 * 24)
  );
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`group outline-none focus:ring-2 focus:ring-primary/20 rounded-xl ${
        isDragging ? "z-50" : ""
      }`}
    >
      <Card
      ...
      >
        ...
        <CardHeader className="p-4 pb-2 pl-5">
          ...
          <div className="flex justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              {isOverSLA ? (
                ...
                <span
                  className={`text-[9px] font-bold uppercase tracking-tight flex items-center gap-1 ${
                    isNearSLA ? "text-orange-500" : "text-slate-400"
                  }`}
                >
          ...
          {/* Title */}
          <CardTitle className="text-[13px] font-bold text-slate-800 leading-tight mt-2 group-hover:text-primary transition-colors">
            {vaga.titulo}
          </CardTitle>
        </CardHeader>
        ...
        <CardContent className="p-4 pt-0 pl-5">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {vaga.squad_destino && (
              <>
                <span className="w-1 h-1 rounded-full bg-slate-200 shrink-0" />
                <span className="text-[10px] font-semibold text-primary/60 bg-primary/5 border border-primary/10 px-1.5 py-0 rounded-md">
                  {vaga.squad_destino}
                </span>
              </>
            )}
          ...
          <div className="mt-4 flex items-center justify-between border-t border-slate-50 pt-3">
            ...
            {candidatosCount > 0 && (
              <div className="flex items-center gap-1 text-slate-400">
                ...
                <span className="text-[10px] font-bold">{candidatosCount}</span>
              </div>
            )}
          </div>
